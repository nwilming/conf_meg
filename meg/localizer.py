...
try:
    params = tfr.params_from_json('/home/nwilming/conf_analysis/all_tfr150_parameters.json')
except IOError:
    params=None
...
def select(snum, reject=dict(mag=4e-12), params=params):
    ...
    power, power_std = get_localizer_power(snum, reject=reject, params=params).crop(tmin=-0.4, tmax=0.75)
    ...
    plt.savefig('/home/nwilming/conf_analysis/localizer_results/lr_%i_plot.png'%snum, bbox_inches="tight")
...
def select_by_gamma(snum, reject=dict(mag=4e-12), params=params, n=10):
    ...
    import cPickle
    n_sensors = 1
    power, power_std = get_localizer_power(snum, reject=reject, params=params, sensors=['occipital', 'posterior'])
    ...
    mapping = dict((chn[k], k) for k in ordering if chn[k] in ta.sensors['occipital'](chn))
    ordering = [channel for channel in ordering if chn[channel] in mapping.keys()]

    pos = np.asarray([ch['loc'][:3] for ch in power.info['chs']])
    N = neighbors.kneighbors_graph(pos, n_neighbors=n_sensors, include_self=True)

    clusters = [np.where(N[i].toarray())[1] for i in ordering[-n:]]
    ch_names, f_roi, freq_resp, dists = [], [], [], []
    for cluster in clusters:
        fresp = (averages.data[cluster, :, :]/power_std[cluster, :,:]).mean(0)
    ...
    plt.figure(figsize=(15,30))
    make_selection_plot2(averages, clusters, ch_names, f_roi, freq_resp, dists, chn)
    plt.savefig('/home/nwilming/conf_analysis/localizer_results/lrbg_%i_plot.png'%snum, bbox_inches="tight")
...
def get_localizer_power(snum, reject=dict(mag=4e-12), params=params, sensors=None):
    ...
    if sensors is not None:
        ...
        epochs.drop_channels([ch for ch in epochs.ch_names if ch not in allowed_sensors])
...
def cluster_sensors(power, n_clusters=5, freq=slice(5, 200)):
    ...
    dims = data.shape
    dists = pairwise.pairwise_distances(data.reshape(dims[0], dims[1]*dims[2]), metric='l2')

    N = neighbors.kneighbors_graph(pos, n_neighbors=4)
    cl = cluster.AgglomerativeClustering(n_clusters=n_clusters,  linkage='average', connectivity=N)
...
def make_selection_plot2(power, clusters, ch_names, f_roi, freq_resp, dists, all_channels):
    ...
    for i, (channels, fr, pf) in enumerate(zip(clusters, freq_resp, f_roi)):
        plt.subplot(gs[i+1,0])
        groups = [channels, list(set(np.arange(len(all_channels)))-set(channels))]
...
def plot_tfr(averages, picks):
    ...
    ratio = freq[1:] / freq[:-1]
    log_freqs = np.concatenate([[freq[0] / ratio[0]], freq, [freq[-1] * ratio[0]]])
...
def detect_eyeartifact(times, xx, yy, time=(-.2, 0.5), Hz=1000, ppd=45.):
    ...
    for x, y in zip(xx, yy):
        ...
        saccades = artifacts.saccade_detection(x[idt]/ppd, y[idt]/ppd, Hz=Hz)
        if len(saccades)>0:
            arts.append(True)
        else:
            arts.append(False)
...
def plot_sensors(info, ch_groups=None):
    ...
    if ch_groups is not None:
        ...
        for i, group in enumerate(ch_groups):
            plt.plot(pos[group, 0], pos[group, 1], 'o', label='C %i'%i, mec='none', color=colors[i])
