'''
Make a model that predicts choice and confidence based on individual contrast samples.
'''
import matplotlib
import sys
...
def variance_model(offset, varslope, a, *tslope, **kwargs):
    ...
    transformed = weights[np.newaxis,:] * transformed
    stdc = (transformed.std(1))/ transformed.std(1).mean() #factor #(weights.mean())#factor
...
def err(modelargs, **kwargs):
    ...
    v =  -np.sum((1-kwargs['responses'])*np.log(1-yy) + kwargs['responses']*np.log(yy))
    if np.isnan(v):
        print modelargs, v
        1/0
    return v#+v
...
def conf_err(modelargs, **kwargs):
    ...
    # r are the confidence responses in [0, 1, 2, 3], i.e. [high no, low no, low yes, high yes]
...
def plot_model(df, model=None, bins=[np.linspace(0, .3, 26), np.linspace(0, 1, 26)],
    ...
    if model is not None:
        if not predicted_surface:
            ...
            for i in range(N):
                ...
                resp1 += plt.histogram2d(df[fieldy].values[idx], df[fieldx].values[idx],
                    bins=bins)[0]
                resp2 += plt.histogram2d(df[fieldy].values[~idx], df[fieldx].values[~idx],
                    bins=bins)[0]
    ...
    if predicted_surface:
        pcol=plt.contour(bins[1][1:], bins[0][1:], np.ma.masked_invalid(plane), [0.35, 0.5, 0.65])
...
def tovarparams(df):
    offset, varslope, a = df.loc[:, 'offset'], df.loc[:, 'std'], df.loc[:, 'alpha']
    time = np.vstack(df.loc[:, 'time'])
    return np.concatenate((offset[:, np.newaxis], varslope[:, np.newaxis], a[:, np.newaxis], time), axis=1)


def tobaseparams(df):
    offset, a = df.loc[:, 'offset'], df.loc[:, 'alpha']
    time = np.vstack(df.loc[:, 'time'])
    return np.concatenate((offset[:, np.newaxis], a[:, np.newaxis], time), axis=1)
...
def plot_subject(d, df, start=1000, row=None, gs=None):
    ...
    for j, var in enumerate([True, False]):
        try:
            ...
            for t, weights in enumerate(w.T):
                ...
                plt.plot([1+t+j/3.,1+t+j/3.], np.percentile(weights, [5, 95]), color=colors[j], dash_capstyle='round', lw=5)
                plt.plot([1+t+j/3.], np.percentile(weights, [50]), 'o', color=colors[j])
        except ValueError:
            pass
    plt.xlim([.5, 10.5])
    ...
    plt.plot(np.percentile(df.query('var==True')['alpha'], [5, 95]),  [-.25, -.25],'b', dash_capstyle='round', lw=5)
    plt.plot(np.percentile(df.query('var==True')['alpha'], [50]), [-.25], 'bo', label='Alpha Var')
    try:
        plt.plot(np.percentile(df.query('var==False')['alpha'], [5, 95]), [-0.5, -0.5], 'r', dash_capstyle='round', lw=5)
        plt.plot(np.percentile(df.query('var==False')['alpha'], [50]), [-0.5], 'ro', label='Alpha Base')
    except:
        pass
    plt.plot(np.percentile(df.query('var==True')['std'], [5, 95]), [-.1, -.1], 'g', dash_capstyle='round', lw=5)
    plt.plot(np.percentile(df.query('var==True')['std'], [50]), [-.1], 'go', label='STD')
    plt.xlim([-0.5, 1.5])
    plt.ylim([-0.75, 7.5])
    plt.yticks([])
    plt.xticks([-0.5, 0, 1.])
...
def model_comparison_plot(resdf, d):
    ...
    for i,row in resdf.iterrows():
        ...
        cnt+=1
        plot_model(kd,
                cmap='RdBu_r', bins=[np.linspace(0,0.3,11), np.linspace(low, high, 11)])
        plt.xticks([0.5])
        plt.yticks([])
        plt.subplot(8, 2*(nmodels+2), cnt)
...
def get_all_subs(d, models=[True, False], iter=1000):
    res = []
    for s, kd in d.groupby('snum'):
       for var in models:
            with bayes_variance_model(kd, var=var):
                trace = pm.sample(iter, init='advi',
                    njobs=3)
            print s, pm.diagnostics.gelman_rubin(trace)
            dd = {'snum':s, 'var':var}
...
def parameter_overview(d):
    ...
    for j, std in enumerate(np.linspace(-.25, .25,9)):
        for i, alpha in enumerate(np.linspace(-1.5, 1.5, 9)):
            ...
            plot_model(d, lambda x: variance_model(*params, contrast=x), cmap='RdBu_r',
                bins=[np.linspace(0,0.25,26), np.linspace(.3, .7, 26)],
    ...
    plt.tight_layout()
    plt.savefig('/Users/nwilming/u/conf_analysis/plots/prameter_overview.pdf', bbox_inches='tight')
    plt.savefig('/Users/nwilming/u/conf_analysis/plots/prameter_overview.svg', bbox_inches='tight')
    ...
    for (s, v), d in df.groupby(['snum', 'var']):
        for i, param in enumerate([ 'std', 'alpha', 'offset']):
            ...
            if (i==2) and (s==1):
                ...
                plt.fill_between(x, y0, y1, alpha=0.5, color=varcol[v], label=labels[v])
            else:
