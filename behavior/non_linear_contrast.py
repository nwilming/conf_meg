...
def make_R(a=1, p=0.2, q=1.5, sigma=2.5):
    return lambda C: a*((100*C)**(p+q))/((100*C)**q + sigma**1)

R = make_R(**parameters['djh_V1'])


def plot(data, R, bins, fit_cutoff=0.3,
            expected_mean_bins=[np.linspace(0.01, 0.7,31), np.linspace(2.8, 4.2,101)],
            expected_mean_data=None):
    dt = transform(R, data)
    log_res, results = empirical.fit_logistic(data.query('trans_stdc<%f'%fit_cutoff), 'R ~ trans_mc + trans_stdc', summary=False)
    _ = plot_model(data, results, bins=bins, cmap='RdBu_r', mc_field='trans_mc',
        stdc_field='trans_stdc', plot_hyperplane=False)
...
def plot_model(df, model, bins=[np.linspace(0,.25,100), np.linspace(0,1,100)],
            ...
            mc_field='mc', stdc_field='stdc',
            plot_hyperplane=True):
    C, M = plt.meshgrid(*bins)
    resp1 = plt.histogram2d(df[df.response==1].loc[:, stdc_field].values,
                        df[df.response==1].loc[:, mc_field].values, bins=bins)[0] +1
    resp1[resp1==1] = np.nan
    resp2 = plt.histogram2d(df[df.response==-1].loc[:, stdc_field].values,
                        df[df.response==-1].loc[:, mc_field].values, bins=bins)[0] +1
    ...
    decision = lambda x: -(model.params[mc_field]*x+ model.params.Intercept)/model.params[stdc_field]
    if not hyperplane_only:
        plane = np.log(resp1/resp2)
        plane[plane==1] = np.nan
        plt.pcolormesh(bins[1], bins[0], np.ma.masked_invalid(plane), cmap=cmap, vmin=-2.4, vmax=2.4)
...
def expected_mean(data, bins, levels=[0.5]):
    ...
    CS = plt.contour(center(bins[1]), center(bins[0]), np.ma.masked_invalid(expected), levels, colors='m')
    zc = CS.collections[0]
