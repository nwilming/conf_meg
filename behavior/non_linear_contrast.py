...
def plot_wotrans(data, R, bins, fit_cutoff=0.3):
    ...
    log_res, data_results = empirical.fit_logistic(data, 'R ~ mc + stdc', summary=False)

    log_res, results = empirical.fit_logistic(data, 'true_mc ~ trans_stdc + trans_mc', summary=False)
    data = data.copy()
    ...
    stuff= plot_model(data, results, bins=bins, cmap='RdBu_r', plot_hyperplane=True)
    plot_model(data, data_results, bins=bins, cmap='RdBu_r', hyperplane_only=True, plot_hyperplane=True)
    return stuff
...
def plot(data, R, bins, fit_cutoff=0.3,
    ...
    dt = transform(R, data)
    log_res, results = empirical.fit_logistic(data, 'R ~ trans_mc + trans_stdc', summary=False)
    print(results.params)
