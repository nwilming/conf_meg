...
#        print d[indexs[0]]
        ...
        for iboot in range(Nboot):
            ...
            area_boot[iboot] = total_area_kernel_PInormalize(
                aux_kernel.T[iboot])
...
def rate2rateframes(stim, Nframes):
    if len(stim) % Nframes != 0:
        ...
        for iframe in range(Nframes):
            stim_f[iframe] = np.mean(
                stim[iframe * window:(iframe + 1) * window])
...
def total_area_kernel_PInormalize(kernel):
    ...
    '''
    nframes = len(kernel)
    area_pi = nframes * (0.5 + 2 / np.pi * np.arctan(1 /
                                                     np.sqrt(2 * nframes - 1))) - 0.5 * nframes
...
def trap(a1, a2, mu, a, b, D, n):
    h = (a2 - a1) / (n - 1)
    value = 0.5 * (np.exp(potential(a1, mu, a, b) / D) +
                   np.exp(potential(a2, mu, a, b) / D))
    for i in range(n):
...
def Performance_sigma(sigmas_theory, coef, t=100.0, sigma0=0.0):
    ...
    for isigma in range(len(sigmas_theory)):
        ...
        zc = np.max(z)
        zb = np.median(z)
        PR0[isigma] = trap(za, x0, mu, a, b, D, Ntrap) / \
            trap(za, zc, mu, a, b, D, Ntrap)
        Delta_ab = potential(zb, mu, a, b) - potential(za, mu, a, b)
        Delta_cb = potential(zb, mu, a, b) - potential(zc, mu, a, b)
        # Rate from ac and ca
        aux_kac = np.sqrt(np.abs(potential_prima2(zb, a, b)
                                 * potential_prima2(za, a, b))) / (2.0 * np.pi)
        kac = aux_kac * np.exp(-Delta_ab / D)

        aux_kca = np.sqrt(np.abs(potential_prima2(zb, a, b)
                                 * potential_prima2(zc, a, b))) / (2.0 * np.pi)
        kca = aux_kca * np.exp(-Delta_cb / D)

        PRs[isigma] = kac / (kac + kca)
        PRR[isigma] = PRs[isigma] + \
            np.exp(-(kca + kac) * t) * (1 - PRs[isigma])
        PRL[isigma] = PRs[isigma] * (1 - np.exp(-(kca + kac) * t))
        PR[isigma] = PRR[isigma] * PR0[isigma] + \
            PRL[isigma] * (1 - PR0[isigma])
