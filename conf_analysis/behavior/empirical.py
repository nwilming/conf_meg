...
def load_data():
    ...
    for f in files:
        ...
        df = pd.DataFrame([dict((k, unbox(i[k].ravel()))
                                for k in m.dtype.fields) for i in m])
        subject = int(f.split('/')[5][1:3])
        df['snum'] = subject
        df['trial'] = np.arange(len(df))
        dfs.append(df)
    data = pd.concat(dfs).reset_index()
    day = [int(time.strftime('%Y%m%d', time.strptime(k, '%Y%m%dT%H%M%S')))
    ...
    def block_num(data):
        lt = dict((k, i) for i, k in enumerate(
            np.sort(np.unique(data.session.astype('str')))))
    ...
    data = data.groupby(['snum', 'session_num', 'block_num']
                        ).apply(contrast_block_mean)
...
def conf_kernels(df, alpha=1, rm_mean=False, label=True, err_band=False):
    ...
    colors = dict((k, c) for k, c in zip(
        ((-1., 2.), (-1., 1.), (1., 2.), (1., 1.)), sns.color_palette(n_colors=4)))
    ...
    for cond, group in df.groupby(['response', 'confidence']):
        ...
        if err_band:
            ...
            plt.fill_between(x, band[:, 0], band[:, 2],
                             color=colors[cond], alpha=alpha)
        if label:
            plt.plot(x, kernel.mean(0), label=legend_labels[
                     cond], color=colors[cond], alpha=alpha)
        else:
...
def plot_model(df, model, bins=[np.linspace(0, .25, 100), np.linspace(0, 1, 100)],
    ...
    decision = lambda x: -(model.params.mc * x +
                           model.params.Intercept) / model.params.stdc
    if not hyperplane_only:
        plane = np.log(resp1 / resp2)
        plane[plane == 1] = np.nan
        pcol = plt.pcolormesh(bins[1], bins[0], np.ma.masked_invalid(
            plane), cmap=cmap, vmin=-2.4, vmax=2.4, rasterized=True, linewidth=0)
