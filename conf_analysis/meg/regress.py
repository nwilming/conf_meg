...
def eval_stim_latency(cache_key, motor_latency=1.4, baseline=False, latency_stim=np.arange(0, 0.4, 1/60.), n_jobs=8):
    from joblib import Parallel, delayed
    import pickle
    args = list(delayed(eval_all_subs)(motor_latency, i, baseline) for i in latency_stim)
    results = Parallel(n_jobs=8)(args)
    sc = []
    weights = {}
    shuff_weights = {}
    iweights = {}
    for j, i in enumerate(np.arange(0, 0.4, 1/30.)):
    ...
    pickle.dump(
        {'scores': scores, 'weights':weights, 'shuffled_weights':shuff_weights, '1smp_weights':iweights}, 
        open('/home/nwilming/conf_analysis/results/%s_stim_latency.pickle'%cache_key, 'wb'))
...
def eval_all_subs(latency_motor=1.4, latency_stim=0.18, baseline_correct=False):
    ...
    for subject in range(1, 16):
        s, w, ss, sw, si, iw = eval_coupling(subject, latency_motor=latency_motor, 
            latency_stim=latency_stim, baseline_correct=baseline_correct)
        scores.append(s)
        weights.append(w)
        ...
        iscores.append(si)
        iweights.append(iw)
    scores = pd.DataFrame({'corr':scores, '1smp_corr':iscores, 'shuff_corr':shuffled_scores})
    scores.loc[:, 'subject'] = np.arange(1, 16)
    return scores, weights, sweights, iweights
    

def eval_coupling(subject, latency_motor=1.4, latency_stim=0.18, baseline_correct=False):
    motor, evals = get_motor_prediction(subject, latency_motor, cluster="JWG_M1")
    ...
    score, weights, intercept = coupling(lodds, X, n_iter=50, pcdist=sp_randint(5, 40))
    shuffled_score, shuffled_weights, s_intercept = coupling(
        shuffle(lodds), X,  n_iter=50, pcdist=sp_randint(5, 40),
    )
    inst_corr, iweights, _ = coupling(
        lodds, X[:, -2:],  n_iter=50, pcdist=None
    )
    return score, weights, shuffled_score, shuffled_weights, inst_corr, iweights
...
def build_design_matrix(
    subject, trial_index, cluster, latency, add_contrast=False, zscore=True,
    freq_bands=None
):
    filenames = glob(join(inpath, "S%i_*_%s_agg.hdf" % (subject, "stimulus")))
    data = asr.delayed_agg(filenames, hemi="Averaged", cluster=cluster)()
    X, time_per_col = prep_low_level_data(cluster, data, 0, latency, trial_index,
    ...
    for s in np.arange(0, 1, 0.1) + latency:
        ...
        for a in da.ensure_iter(areas):
            ...
            if freq_bands is not None:
                x = x.T.groupby(
                    pd.cut(x.T.index, freq_bands, 
                    labels=np.array(freq_bands)[:-1] +  np.diff(freq_bands))).mean().T
